...
if($username){			
	...
	$sql = "SELECT * FROM letters_of_happiness WHERE SENDER = ? ";
	...
	if($stmt1->rowCount()){
		...
		$sql = "SELECT * FROM letters_of_happiness WHERE SENDER = ? ORDER BY TIME DESC  LIMIT ".$start_rec.",".$per_page;
		...
		$datas = '<div class="blink errors">Sorry you have not submitted any testimonies yet Please <a href="loh" class="links">click here</a> to give a testimony now.</div>';
	}
...
<div class="wrapper">
	...
	<header class="mainnav">
		...
		$page_self = getReferringPage("qstr url");
			...
			echo "<a href='".$page_self."'>My Testimonies </a> "  ;
	...
	<div class="view_user_wrapper" id="hide_vuwbb">
		...
		<?php 
			...
			echo '<h1 class="h_bkg2">  TESTIMONIES BY <span class="lgreen">'.strtoupper($username).' </span> <br/>'.$curr_page.'</h1>';
			
			if($pagination) echo $pagination;
			if($datas) echo '<div class="loh"><h2 class="cyan">TESTIMONIALS</h2>
								'.$datas.'
							</div>';
			if($pagination) echo $pagination;			
